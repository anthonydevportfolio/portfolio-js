    ...
    return (
        <div className='linksBox'>
            <div className='links-social'>
                {socials.map(social => (
                    <Img type={'social'} link={social} />
                ))}
            </div>
            <div className='links-languages'>
                {languages.map(language => (
                    <Img type={'language'} link={language} />
                ))}
            </div>
        </div>
    );
...
const Img = ({ link, type }) => {
    return (
        <img
            src={link[0]}
            className={`introIcon ${type}Icon`}
            alt='language icon ts, js, java'
            onClick={() => {
                if (type === 'social') {
...
const socials = [
    ...
    [
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS4h4yf5vhuu8_Dqf5VC1l1tFbIJ88N4H24jg&s',
        'https://leetcode.com/u/Anthony4834/'
    ]
];
const languages = [
